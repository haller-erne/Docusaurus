...
- Rexroth Nexo and Nexo 2 wireless tools (for more information, see [Nexo OpenProtocol](nexo.mdx))
...
The `OpenProtocol` driver is implemented in `OpConn.dll`. To use any `OpenProtocol` tool, the driver must be loaded in the `[TOOL_DLL]` section of the projects `station.ini` configuration file (see also [Tool configuration](../index.md)).
...
Note, that the signal is set to high (block loosen) only, if the current logged on user does not have the user right `CCW` assigned (see [OGS user rights](../../reference/lua/interfaces/userrights.md) for more information).
...
OGS has two sepperate user rights related to loosen (see also [OGS user rights](../../reference/lua/interfaces/userrights.md) and [NOK behaviour](#nok-behaviour-and-rework-parameters)):
...
Defines the time [in minutes] when OGS shall check the tools clock. This setting is only used, if time synchronization is enabled for the tool (see [CHANNEL_[TOOL]_CHECK_TIME_ENABLED](#check_time_enabled) below).
...
#### CHANNEL_[tool]_ALIVEXMTT {#channel__alivexmtt}
...
#### CHANNEL_[tool]_RSPTIMEOUT {#channel__rsptimeout}
_(optional, default defined by tool type (see above))_
...
See [System 350 OpenProtocol](sys350-ke350.md) for more info about multi-spindle mode and how to set it up.
...
#### CHANNEL_[tool]_CHECK_TIME_ENABLED {#check_time_enabled}
_(optional, default = 0 (disabled))_
