...
This page describes the specifics for configuring the Makita 18V wifi battery tools to work with OGS, for general information about the OGS OpenProtocol configuration, see [OpenProtocol Tools](index.md).
...
<img style={{float: 'right', margin: '0 0 1rem 1rem'}} width="200" src={require('./resources/makita.png').default} alt="Makita tools" />
...
![alt text](resources/makita-wifi-info.png)
![alt text](resources/makita-wifi.png)
...
Station.ini configuration uses the standard parameters of the OpenProtocol driver (see [OpenProtocol Tools](index.md)).
...
Even though there are more options (see the general discussion in [OpenProtocol Tools - Loosen modes](index.md#loosen-modes)), it is recommended to setup the tool according to what is described above. Ogs then actually only enabled the tool for a single clockwise rundown (loosen is blocked before). If an Nok happens, then the tool does not report it back to OGS before a loosen run is done (by switching the start switch to CCW and starting the tool using the start button).
