...
- As a pdf file creator (or sheet printout) at the end of the workflow to
  ...
  report (with [Databanking](../admin/databanking-and-rework.md))).
...
As the functionality for automatic printing is shared with the XML file output (see [End-of-process (XML) file](../reference/data-output/xml-file-format.md)), make sure to add the `print` module to the end of the `requires = {}`-list in `config.lua`!
...
functionality as XML-file creation, see the documentation at [End-of-process (XML) file - Define when a file is generated](../reference/data-output/xml-file-format.md#define-when-a-file-is-generated) for more details.
