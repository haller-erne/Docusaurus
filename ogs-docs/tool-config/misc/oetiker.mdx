...
<img style={{float: 'right', margin: '0 0 1rem 1rem'}} src={require('./resources/oetiker-pincer-epc01.jpg').default} alt="Oetiker EPC01 Pincer" />
...
As the Keyence IV4 AI Smart Sensor driver is implemented as LUA custom tool, the instuction provided in the [Lua custom tools documentation](../../reference/lua/custom-tools.md) applies. A sample configuration for the lua driver `camera_keyence` in `station.ini` looks as follows:
...
The driver for the Oetiker EPC01 controller is automatically loaded.
  ...
  <TabItem value="lt-v32" label="< V3.2">
