...
<img style={{float: 'right', margin: '0 0 1rem 1rem'}} src={require('./resources/wenglor-weqube-b50.png').default} alt="Wenglor weQube Smart Camera" />
...
As the Wenglor weQube Smart Sensor driver is implemented as LUA custom tool, the instuction provided in the [Lua custom tools documentation](../../reference/lua/custom-tools.md) applies. A sample configuration for the lua driver `camera_wenglor` in `station.ini` looks as follows:
...
<Tabs>
  <TabItem value="-v316" label=">= V3.1.6">
...
The driver for the camera is automatically loaded.
  ...
  <TabItem value="lt-v316" label="< V3.1.6">
