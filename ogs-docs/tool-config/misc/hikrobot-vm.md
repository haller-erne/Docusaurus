...
<img style={{float: 'right', margin: '0 0 1rem 1rem'}} src={require('./resources/hikrobot-vm.png').default} alt="HikRobot VisionMaster" />
...
As the Hikrobot VisionMaster driver is implemented as LUA custom tool, the instuction provided in the [Lua custom tools documentation](../../reference/lua/custom-tools.md) applies. A sample configuration for the lua driver `camera_hikrobot_vm` in `station.ini` looks as follows:
