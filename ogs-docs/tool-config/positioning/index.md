...
### Project configuration {#workflow-configuration}
...
To add a positioning system to a LUA tool (see [LUA custom tools](../../reference/lua/custom-tools.md)), a slightly different syntax is needed. In this case, the `POSITIONING=<section>` must be added to the tools configuration section. Here is a sample:
...
- Sarissa PositionBox (documentation pending)
