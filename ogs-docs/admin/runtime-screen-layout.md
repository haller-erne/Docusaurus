---
title: Runtime screen layout
sidebar_label: Runtime screen layout
---
...
The web view shows a custom web page served through the [OGS integrated webserver](../reference/lua/interfaces/webserver.md) but can also show any other web page.

The url for the webpage is defined in the `current_project.billboard` value in the projects `config.lua` file. If the value is a full URL (including the protocol prefix), then the URL is used verbatim. If only a file name is given and the [OGS integrated webserver](../reference/lua/interfaces/webserver.md) is enabled, the file is served from the default webroot.
...
For generic branding, see [branding](../reference/branding.md).
